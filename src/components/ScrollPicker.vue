<template>
    <div class="scroll-picker-container" ref="self">
        ...
        <div v-for="(column, i) in options" :key="`column-${i}`" class="column-container">
            <div class="column-overlay" :id="`column-${i}`" :ref="el => setColumnRef(el, i, 'overlay')" />
            <div :id="`column-${i}`" class="column-content" :ref="el => setColumnRef(el, i, 'content')">
                <div class="pad" :style="padStyle" />
                <div v-for="(row, j) in column" :key="`row-${j}`" :id="`row-${j}`" class="row-option"
                    :style="getIsActive(row, i) ? activeStyle : inactiveStyle" :class="getIsActive(row, i)
